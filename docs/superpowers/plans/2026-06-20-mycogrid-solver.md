...
- **Performance cliff on sparse / under-clued grids (CONFIRMED, deferred)** — propagation handles the curated pool fast (≤5 guesses/puzzle), but the final review measured that under-clued inputs blow up: a 5×7 grid with a single clue takes ~13s, and a 7×10 grid with a single clue does not finish within 60s. Cause: once `chooseUnknown` exhausts clued-cell edges it falls back to the first unconstrained unknown with no subloop pruning. This is documented in a `KNOWN LIMIT` comment above `chooseUnknown` in `Solver.swift`. It does **not** affect this sub-project (its only live input is the well-clued curated pool), so it was deliberately NOT fixed here to avoid destabilizing a verified-correct solver. **Handoff to the generator sub-project:** before relying on `solve()` for arbitrary/under-clued candidates, add subloop pruning (discard branches that seal a loop while clues remain) and/or a guess/time budget that returns an indeterminate signal.
